...
sourceFile <- "analyze_liver_v0.2.R"
...
blast.data.2$id <- gsub("_seq.*","",gsub(" .*","",blast.data.2$NMR_ID))

mean(blast.data.2$id %in% nmr.liver.id.normed$id)

blast.data.nodup <- blast.data.2[order(-blast.data.2$Pct.ID),]
blast.data.nodup <- subset(blast.data.nodup,!duplicated(id))

merge.normed <- merge(blast.data.nodup,nmr.liver.id.normed,by="id")

for(namer in grep("NMR_liver",names(merge.normed),value=TRUE)){
	merge.normed[[namer]] <- ifelse(merge.normed[[namer]]==-Inf,-1,merge.normed[[namer]])
	}

# compute intergenic correlations

x <- subset(merge.normed,symbol=="Dnajc12")
data.mat <- t(as.matrix(x[,grep("NMR_liver",names(x),value=TRUE)]))


corinGene <- function(x){
	
	if(nrow(x)==1){mean.cors <- NA}
	
	if(nrow(x)>1){
	data.mat <- t(as.matrix(x[,grep("NMR_liver",names(x),value=TRUE)]))
	cors <- cor(data.mat,use="pair")
	diag(cors) <- NA
	mean.cors <- rowMeans(cors,na.rm=TRUE)	
	
	}
	x$mean.corr <- mean.cors
	return(x)
	
} #


ugenes <- unique(merge.normed$Symbol)
...
for(gene in ugenes[1:1000]){
	
	gene.data <- subset(merge.normed,Symbol==gene)
	
	corlist[[gene]] <- corinGene(gene.data)
	
	step <- step + 1
	
	all.data <- rbind(all.data,corlist[[gene]])
	
	print.skip(step,1000,percent=5)
	}


all.cors <- all.data

pdf(pastes(resultsdir,"with_transcript_corr_pctID.pdf"),h=8,w=11)

number.subseqs <- table(merge.normed$symbol)

temp <- hist(number.subseqs[number.subseqs<21],main="NMR Subseq's per mouse gene",xlim=c(1,20),breaks=20)

hist(all.cors$mean.corr,main="Distribution of within transcript Corr",xlab="Within Transcript Correlation")

plot(data.mat,xlab="Log Counts Subseq 1",ylab="Log Counts Subseq 2",main="Mouse gene with 2 NMR Subseqs",pch=19)

legend("topleft",legend=pastes("Corr=",round(cor(data.mat[,1],data.mat[,2]),2)))

with(all.cors,plot(Pct.ID,mean.corr,main="Mean within transcript correlation vs. % ID with mouse Gene",pch=19))

abline(h=0,col="blue",lwd=3,lty=2)

xy.d <- na.exclude(data.frame(x=all.cors$Pct.ID,y=all.cors$mean.corr))
xy.d <- xy.d[order(xy.d$x),]
loessFit <- loess(y~x,data=xy.d)
lines(xy.d$x,predict(loessFit),pch=19,col=2,lwd=2)


dev.off()

cor.test(xy.d$x,xy.d$y)
...
all.genes <- unique(nmr.liver.data.agg$SYMBOL)
...
for(gene.pick in all.genes[1:n]){
	...
	int.p.vec[gene.pick,1] <- interactions.mus.nmr.by.gene(gene.pick,mus.d=mouse.liver.data.agg,nmr.d=nmr.liver.data.agg,plot.label="liver")
...
write.csv(df.out ,pastes(resultsdir,"liver_most_significant_difference_in_change.csv"),row.names=FALSE)



pdf(pastes(resultsdir,"interaction_p_genes_liver_most_sig.pdf"),h=8,w=11)


for(gene.pick in most.sig[1:100] ){
