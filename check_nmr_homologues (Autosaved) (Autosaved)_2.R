...
ggplot2::qplot(Count.Freq,log10(V3),data=mus2nmr.matches.count,geom=c("point","smooth")) + labs(title="Average ID by Match") + xlab("# Matches") + ylab("Prop ID")
...
pheno.factor <- as.factor(grep.to.label(colnames(tissue.counts),"Liver",1,2))
...
EBOut <- EBTest(Data=tissue.counts,Conditions=pheno.factor,sizeFactors=Sizes, maxround=5)
...
agg.matches <- aggregate(subset(pp.df.mg,select=c("PPDE","EB.FC")),by=subset(pp.df.mg,select="V1"),mean,na.rm=TRUE)
...
with(merge.comp,plot(log2(FC),log2(EB.FC),main="Cluster mean Fold-Change (EBSeq) vs. Fold-Change  Cluster (GEE)"))
