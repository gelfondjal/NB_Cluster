...
merged.single <- Sortby(merged.single,"V3",decreasing=TRUE)

merged.single.nodups <- subset(merged.single,!duplicated(V1))
...
for(namer in names(analyses)){
	...
	bigger.list[[namer]] <- merge(bigger.list[[namer]],subset(matches.2,!duplicated(V1)),by="V1")	
...
all.types.list <- unique(c(as.character(subset(bigger.list[[1]],!is.na(pvalue))$V1),as.character(subset(bigger.list[[2]],!is.na(pvalue))$V1)))

all.results <- subset(all.results,V1 %in% all.types.list)
...
multivariate.only.not.singleton.results <- rename(multivariate.only.not.singleton.results,replace=c("V1" ="ucsc","V2" ="NMR_Gene"))
...
make.latex.doc.vector(pastes(tex.dir,"summary_realdata.tex"),includer=table.files,title="Liver vs. Skin NMR to mouse homologs",author="Jon Gelfond",date=as.character(Sys.time()),path=NULL)

Create.pdflatex("summary_realdata.tex",tex.dir)
