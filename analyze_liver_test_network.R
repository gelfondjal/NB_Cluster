...
sourceFile <- "analyze_liver_test_network.R"
...
both.all <- merge(nmr.all,mouse.all,by="SYMBOL")

all.data <- as.matrix(both.all[,grep("log",names(both.all),value=TRUE)])
rownames(all.data) <- both.all$SYMBOL


top.genes <- apply(all.data,1,sd,na.rm=TRUE)
...
base1 <- "/Users/jonathangelfond/Documents/Projects/Thompson/"
base2 <- "/Users/jonathangelfond/Dropbox/Temp_Work/Thompson/"

basedir <- base1


source.file <- "estimate_B_matrix_ALL_perm_2.R"
...
source("/Users/jonathangelfond/Documents/Projects/sudheer/Analysis/table_functions.R")
source("/Users/jonathangelfond/Documents/Projects/Computing/R_programming/Jitter_Bug/jitter_functions.R")
source("/Users/jonathangelfond/Documents/Projects/Joe_Lab/Network/Analysis/OSRR_functions.R")
source("/Users/jonathangelfond/Documents/Projects/Thompson/Analysis/support_functions.R")

Dbfile <- pastes(datadir,"GSE1133-GPL1073_series_matrix.txt")


dir.create(resultsdir)

maData <- read.delim(Dbfile,skip=74)

dataMatrix <- as.matrix(maData[,-1])

rownames(dataMatrix) <- maData[,1]


phenoData <- t(as.matrix(read.delim(Dbfile,skip=46,nrows=12,header=FALSE,as.is=TRUE)))
phenoNames <- gsub("^!","",phenoData[1,] )
phenoData <- phenoData[-1,]
phenoData <- data.frame(phenoData,stringsAsFactors=FALSE)


names(phenoData) <- phenoNames

identical(as.character(phenoData[,2]),colnames(dataMatrix))
...
dataNorm <- normalize.quantiles(dataMatrix)
dimnames(dataNorm) <- dimnames(dataMatrix)
dataNorm <- log2(dataNorm)

hist(dataNorm)
dataNormF <- dataNorm[!rowMeans(dataNorm<6)>0.5,]


dataNormDb <- dataNorm

phenoDataDb <- phenoData

source(pastes(analysisdir,"read_circadian.R"))

summary(rownames(dataNormDb) == rownames(dataNorm))

sum(colnames(dataNormDb) %in% colnames(dataNorm))


# Get the n=1000 genes that are most variable

DataimputeDb <- impute.knn(dataNormDb)$data
dimnames(DataimputeDb) <- dimnames(dataNormDb)


DataimputeCirc <- impute.knn(dataNorm)$data
dimnames(DataimputeCirc) <- dimnames(dataNorm)


impgenes <- read.csv(pastes(datadir,"selected_genes.csv"),as.is=TRUE)


impRowIDs <- impgenes$"Gene.symbol" # This is questionable


vars <- apply(dataNorm,1,var)

nPick <- 3000

varThreshold <- -sort(-vars)[nPick]

identical(nPick==sum(vars>=varThreshold,na.rm=TRUE),TRUE)

geneSelector <- (vars>=varThreshold)|(rownames(DataimputeCirc) %in% impRowIDs)

geneSelector <- ifelse(is.na(geneSelector),FALSE,geneSelector)

importantPick <- rownames(dataNormDb) %in% impRowIDs

dataDbPick <- DataimputeDb

dataDbPick <- dataDbPick[rownames(dataDbPick)!="!series_matrix_table_end",]
...
annot <- read.delim(pastes(datadir,"GPL1073-10859.txt"),skip=8)


annotIPI <- read.delim(pastes(datadir,"genBank_to_symbol.txt"),skip=1)
annotIPI$Gene <- gsub("\\*","",annotIPI$Gene)

allAnnot <- merge(annotIPI,annot,by.x="ID",by.y="GB_ACC")

names(allAnnot)[1] <- "ID1"

dataDbPick <- dataDbPick[rownames(dataDbPick) %in% allAnnot$ID,]

matches <- cbind(allAnnot$Gene,as.character(allAnnot$ID))[match(rownames(dataDbPick),allAnnot$ID),]

if(mean(matches[,2]==rownames(dataDbPick))==1){rownames(dataDbPick) <- matches[,1]}

sum(duplicated(rownames(dataDbPick)))

dataDbPick <- dataDbPick[!duplicated(rownames(dataDbPick)),]

mean(toupper(gsub(" .*","",rownames(dataDbPick))) %in% toupper(rownames(all.data)))

common.genes <- intersect(rownames(dataDbPick),rownames(all.data))

write.csv(common.genes,pastes(resultsdir,"common_genes.csv"))

dataDbPick <- dataDbPick[rownames(dataDbPick) %in% common.genes,]

dataDbPick <- dataDbPick[order(rownames(dataDbPick)),]


network.genes <- rownames(dataDbPick)


G <- length(common.genes)

x <- apply(dataDbPick,1,scale,scale=FALSE)


random.subset <- common.genes

match.g <- match(random.subset,rownames(dataDbPick))

lambda <- 10^(3)

n.lambda <- length(lambda)
B <- list()
for(l in 1:n.lambda){B[[l]] <- matrix(0,G,G)}
...
for(g in match.g){
	...
	ridge.fit <- lm.ridge(y~xmat,lambda=lambda)
	
	#print(ridge.fit)
	
	if(n.lambda>1){
	
	coef.matrix <- coef(ridge.fit)[,-1] # Remove Intercept
	
	for(l in 1:n.lambda){B[[l]][g,-g] <- coef.matrix[l,]}
	
	}else{
		B[[l]][g,-g] <- coef(ridge.fit)[-1] # Remove Intercept
		}
	
	print.skip(step,itermax=length(match.g),step=1)
	step <- step+1
	
}  # Loop over g

n.test.genes <- length(match.g)
...
mouse.data.pick <- mouse.data[rownames(mouse.data) %in% common.genes,]
nmr.data.pick <- nmr.data[rownames(nmr.data) %in% common.genes,]
...
#nmr.t[is.na(nmr.t)] <- 0
#mouse.t[is.na(mouse.t)] <- 0




identical(rownames(nmr.data.pick),rownames(mouse.data.pick))
identical(rownames(nmr.data.pick),rownames(dataDbPick))


rownames(cors) <- random.subset
...
yhat.mouse <- 	B[[l]] %*% t(mouse.t)

yhat.nmr <- 	B[[l]] %*% t(nmr.t)

	
for(g in match.g){
			
		
cors[match(g,match.g),"mouse"] <- cor(yhat.mouse[g,],mouse.t[,g])
cors[match(g,match.g),"nmr"] <- cor(yhat.nmr[g,],nmr.t[,g])
			
}
}


write.csv(B[[1]],pastes(resultsdir,"mouse_network_nmr_seq.csv"))

osrr.cors <- data.frame(cors)


medians <- colMedians(cors,na.rm=TRUE)
names(medians) <- colnames(cors)

pdf(pastes(resultsdir,"network_comparison.pdf"),h=8,w=8)
...
plot(cors[,"nmr"],cors[,"mouse"],main=paste("Mouse vs. NMR: Correlation with Prediction in",length(match.g)," genes"))
