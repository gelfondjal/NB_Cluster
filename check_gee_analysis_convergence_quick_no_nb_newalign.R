...
tex.list <- list()
workspace.file <- "/Users/jonathangelfond/Documents/Projects/Buffenstein/NMR_Genomics/Documents/Results/check_gee_analysis_Jan_2014_data_update/all.R.Obj.Rdata"
...
names(nmr2mus.new ) <- c("NMR.gene","mus.UCSCID","mus.Gene.Symbol","NMR.length","mus.length",
					"alignment.length","n.mismatches","proportion.homology")
...
# Select mus genes with multiple hits

matches.2.1tomany <- subset(matches.2,(Count.Freq>1)&(Count.Freq<5))

tissue.counts <- tissue.counts.0[matches.2.1tomany$NMR.gene,]
...
tissue.counts.nodups <- tissue.counts[unique(matches.2.1tomany$NMR.gene),]

#fit.gee.out <- fit.gee.4.array(tissue.counts,matches.2.1tomany$V1,pheno.factor,crash.list=c(4300:4400,5300:5400,8300:8400))

size.factors <- estimateSizeFactorsForMatrix(tissue.counts)

tissue.counts.norm <- t(t(tissue.counts)/size.factors)
...
# These fitting function expect V2 to be the transcript and V1 to be the cluster ID
matches.2.1tomany$V1 <- matches.2.1tomany$mus.UCSCID
matches.2.1tomany$V2 <- matches.2.1tomany$NMR.gene
...
analysis.out.perm <- lapply(families,function(x){fit.gee.5.array(tissue.counts.norm,matches.2.1tomany,pheno.factor,crash.list=NULL,family=x,perm=4)})
...
single.data$V2 <- single.data$V1

single.data <- Drop.variable(single.data,"V1")

matches.2$V2 <- matches.2$NMR.gene
matches.2$V1 <- matches.2$mus.UCSCID


merged.single <- merge(single.data,matches.2,by="V2")



merged.summary <- ddply(merged.single,"V1",function(x){
	
	x$mean.p <- exp(mean(log(x$pvalue)+0.00001,na.rm=TRUE))
	
	return(subset(x,!duplicated(V2)))
