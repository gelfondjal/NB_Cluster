...
nmr.to.cp.data.matrix <- as.matrix(subset(nmr.to.hs,select=grep("(Skin.*[0-9])|(Liver.*[0-9])",names(nmr.to.hs))))
rownames(nmr.to.cp.data.matrix) <- as.character(nmr.to.hs$Symbol)
...
pdf(pastes(resultsdir,"mus_to_hs_mapping_discordant_genes.pdf"))

cor.recomp <- matrix(NA,nrow(merged.mappings),2)

colnames(cor.recomp) <- c("mus","hs")

for(gene in 1:nrow(merged.mappings)){
	
	symbol <- merged.mappings$Symbol[gene]
	cor.hs <- cor(yhat.nmr[symbol,],nmr.t[,symbol],use="pair",method="spearman")
	cor.mus <- cor(yhat.nmr.mus[symbol,],nmr.t.mus[,symbol],use="pair",method="spearman")
	
	cor.recomp[gene,"mus"] <- cor.mus
	cor.recomp[gene,"hs"] <- cor.hs	
	
	cond <- (cor.hs>0.7) & (cor.mus<(-0.7))
	...
	plot(yhat.nmr[symbol,],nmr.t[,symbol],main=paste("Human map",symbol,"Cor =",round(cor.hs,2)))
	
	plot(yhat.nmr.mus[symbol,],nmr.t.mus[,symbol],main=paste("Mouse map",symbol, "Cor =",round(cor.mus,2)))
...
par(mfrow=c(1,1))
plot(cor.recomp,main="Human vs. Mouse mapping")
